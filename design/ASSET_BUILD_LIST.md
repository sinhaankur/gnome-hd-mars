...
| Asset | Size | Problem |
|-------|------|---------|
| `warrior.glb` | **243 MB** | Used for BOTH the player hero AND enemies. A quarter-GB single model tinted a few ways = huge bloat, zero silhouette variety, slow load. **Top priority to replace.** |
| `combat_robot.glb` | 25 MB | Orphan/unused-ish borrowed robot. |
| `mech.glb` | 16 MB | Orphan generic mech. |

The whole mech layer collapses to `warrior.glb`. Fixing the roster (§1) is the biggest single win.
...
Orphan mech files to delete once replaced: `combat_robot.glb`, `mech.glb`, `wr_hawk.glb`,
`prowler_hawc.glb`, `prowler_mk2/3.glb`, `hawc_heavy/hover/support/tactical.glb`.
